...
def apply_settings(patcher: FilePatcher) -> None:
    ...
    # Replace the whole DRF-only span so this patch does not depend on the
    # exact contents of those dicts.
    patcher.ensure_regex_replace(
        "src/config/settings/base.py",
        r"REST_FRAMEWORK = \{.*?\n# --- CORS ",
        _dmr_settings_block(),
...
def apply_full_file_replacements(patcher: FilePatcher) -> None:
    ...
    # Router, not django.urls.path: otherwise controllers miss the schema.
    ...
    # The shipped tests assert DRF's envelope and import rest_framework.
